...
from fastapi import APIRouter, Body, Depends, HTTPExeption
...
def create_user(user: UserCreateForm = Body(..., embed=True), database=Depends(connect_db)):
    exists_user = database.querty(User.id).filter(User.email == user.email).one_or_none()
    if exists_user:
        raise HTTPExeption(status_code=status.HTTP_400_BAD_REQUEST, detail='Email already exists')

    new_user = User (
        email = user.email
        password = get_password_hash(user.pasword),
